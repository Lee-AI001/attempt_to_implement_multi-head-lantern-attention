...
def load_model_config(config_path):
    ...
    try:
        ...
        required_keys = ["vocab_size", "num_layers", "d_model", "nhead", "dim_feedforward", "num_genres", "d_c", "d_prime_c", "d_R_h"]
        ...
        model_config = {
            ...
            "d_c": model_arch["d_c"],
            "d_prime_c": model_arch["d_prime_c"],
            "d_R_h": model_arch["d_R_h"]
        }
        
        # Validate d_R_h
        if model_config["d_R_h"] % 2 != 0:
            raise ValueError(f"d_R_h must be even for RoPE, got {model_config['d_R_h']}")
...
def load_model(checkpoint_dir, model_config):
    ...
    try:
        model = StoryTellerTransformer(
            ...
            d_c=model_config["d_c"],
            d_prime_c=model_config["d_prime_c"],
            d_R_h=model_config["d_R_h"]
        )
