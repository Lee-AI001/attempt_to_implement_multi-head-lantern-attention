...
from config import (
    ...
    ROUGE_SAMPLE_SIZE, OPTUNA_EPOCHS, D_C, D_PRIME_C, D_R_H
)
...
save_model_config(
    ...
    PLATEAU_FACTOR, PLATEAU_PATIENCE, USE_OPTUNA, OPTUNA_N_TRIALS, num_genres, D_C, D_PRIME_C, D_R_H
)
...
# Update hyperparameters with Optuna results or defaults (around line 250)
...
d_c = best_params.get('d_c', D_C)
d_prime_c = best_params.get('d_prime_c', D_PRIME_C)
d_R_h = best_params.get('d_R_h', D_R_H)

# Validate d_R_h
if d_R_h % 2 != 0:
    raise ValueError(f"d_R_h must be even for RoPE, got {d_R_h}")

# Update save_model_config call (around line 140)
save_model_config(
    project_dir, BASE_DIR_OUT, PROJECT_NAME, DATA_FILES, VOCAB_SIZE, NUM_LAYERS,
    D_MODEL, NHEAD, DROPOUT, EMBED_DROPOUT, DIM_FEEDFORWARD, MAX_LEN, PADDING_VALUE,
    SLIDING_STEP, MAX_CHUNKS, LEARNING_RATE, WEIGHT_DECAY, BATCH_SIZE, START_EPOCH,
...
# Update model initialization (around line 270)
